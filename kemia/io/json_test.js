...
function testImportReactionWithUndefinedConditionsAndReagentsText() {
    var jreaction = kemia.io.Testdata.jreaction;
    var json_rxn = goog.json.parse(goog.json.serialize(jreaction));
...
function test6ReactionRoundtrip() {
    var console = new goog.debug.Console();
    console.setCapturing(true);
    var logger = goog.debug.Logger.getLogger('kemia.io.jsontest');

    var rxn = buildReactionTwoProducts();
	assertEquals(1, rxn.getPluses().length);
    logger.info(rxn.toString());

    var rxn_json = kemia.io.json.reactionToJson(rxn);

    var rxn2 = kemia.io.json.readReaction(rxn_json);
    logger.info(rxn2.toString());

    var rxn3 = kemia.io.json.readReaction(rxn_json, true);
    logger.info(rxn3.toString());

    for (var i = 0; i < rxn.getReactants().length; i++) {
        assertTrue(goog.math.Coordinate.equals(
        	rxn.getReactants()[i].getCenter(),
        	rxn2.getReactants()[i].getCenter()));
        assertTrue(goog.math.Coordinate.equals(
        	rxn.getReactants()[i].getCenter(),
        	rxn3.getReactants()[i].getCenter()));
    }

    for (var i = 0; i < rxn.getProducts().length; i++) {
        assertTrue(goog.math.Coordinate.equals(
        	rxn.getProducts()[i].getCenter(),
        	rxn2.getProducts()[i].getCenter()));
        assertTrue(goog.math.Coordinate.equals(
        	rxn.getProducts()[i].getCenter(),
        	rxn3.getProducts()[i].getCenter()));
    }

    assertTrue(goog.math.Coordinate.equals(
    	rxn.arrows[0].getCenter(),
    	rxn2.arrows[0].getCenter()));

    assertTrue(goog.math.Coordinate.equals(
    	rxn.arrows[0].getCenter(),
    	rxn3.arrows[0].getCenter()));
}
...
function buildReactionTwoProducts() {
    var rxn1 = new kemia.model.Reaction();
    rxn1.header = 'my header';
    rxn1.reagentsText = 'my reagents';
    rxn1.conditionsText = 'my conditions';
    ...
    mol1.addAtom(a1b);
    mol1.addBond(b1);
    rxn1.addReactant(mol1);


    rxn1.setArrow(new kemia.model.Arrow(new goog.math.Coordinate(1.5, 1.5),
    new goog.math.Coordinate(2.5, 1.5)));

    var mol2 = new kemia.model.Molecule('mol2');
    var a2a = new kemia.model.Atom('C', 3.5, 1);
    var a2b = new kemia.model.Atom('C', 4.5, 2);
    var b2 = new kemia.model.Bond(a2a, a2b);
    mol2.addAtom(a2a);
    mol2.addAtom(a2b);
    mol2.addBond(b2);
    rxn1.addProduct(mol2);

    var mol3 = new kemia.model.Molecule('mol3');
    var a3a = new kemia.model.Atom('C', 5.5, 1);
    var a3b = new kemia.model.Atom('C', 6.5, 2);
    var b3 = new kemia.model.Bond(a3a, a3b);
    mol3.addAtom(a3a);
    mol3.addAtom(a3b);
    mol3.addBond(b3);
    rxn1.addProduct(mol3);

	rxn1.generatePluses(rxn1.getProducts());
    return rxn1;
}
