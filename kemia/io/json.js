...
kemia.io.json.moleculeToJson = function(mol) {
    ...
    var atoms = goog.array.map(mol.atoms,
    function(a) {
        return {
            'symbol': a.symbol,
            'coord': {
    ...
    var bonds = goog.array.map(mol.bonds,
    function(b) {
...
kemia.io.json.readReaction = function(arg, opt_permit_overlap) {
    ...
    if (jrxn['arrows']) {
        goog.array.forEach(jrxn['arrows'],
        function(arrow) {
    ...
    if (jrxn['pluses']) {
        goog.array.forEach(jrxn['pluses'],
        function(plus) {
            rxn.addPlus(kemia.io.json.readPlus(plus));
        });
    }

    goog.array.forEach(reactants,
    function(mol) {
        rxn.addReactant(mol);
    });
    ...
    if (jrxn['is_hidden_conditions_text']) {
        rxn.isHiddenConditionsText = true;
    } else {
        rxn.isHiddenConditionsText = false;
    }

	if (jrxn['is_hidden_reagents_text']){
		rxn.isHiddenReagentsText = true;
	} else {
		rxn.isHiddenReagentsText = false;
	}
...
kemia.io.json.reactionToJson = function(rxn) {
    ...
    return {
        ...
        'arrows': arrows,
        'pluses': pluses, 
		'is_hidden_conditions_text' : rxn.isHiddenConditionsText,
		'is_hidden_reagents_text' : rxn.isHiddenReagentsText
    };
};
