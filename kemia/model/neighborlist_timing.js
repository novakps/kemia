...
function runTiming() {
	...
	while (i > 0) {
		...
		var a1 = new kemia.model.Atom("C", Math.random() + center_x, Math
				.random()
				+ center_y);
		var a2 = new kemia.model.Atom("C", Math.random() + center_x, Math
				.random()
				+ center_y);
		var a3 = new kemia.model.Atom("C", Math.random() + center_x, Math
				.random()
				+ center_y);
	...
	var tracer = goog.debug.Trace.startTracer('neighbor list containing '
			+ mols.length + ' molecules');
	var nl = new kemia.model.NeighborList(kemia.model.NeighborList
			.moleculesToNeighbors(mols), 2, .25);
	...
	goog.debug.Trace.addComment('getNearestList called ' + repeat_count
			+ ' times');
	// Stop the trace.
