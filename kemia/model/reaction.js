...
kemia.model.Reaction.prototype.addReactant = function(mol) {
    ...
    if (!this.isReactant(mol)) {
        ...
        goog.array.forEach(this.arrows,
        function(arrow) {
            arrow.translate(diff);
        });
        goog.array.forEach(this.pluses,
        function(plus) {
            plus.translate(diff);
        })
        // move products, since arrow moved
        goog.array.forEach(products,
        function(mol) {
            mol.translate(diff);
        })
    }
    ...
    // force plus generation, since we've moved the components
...
kemia.model.Reaction.prototype.addProduct = function(mol, opt_permit_overlap) {
    ...
    if (this.arrows.length == 0) {
        ...
        if (reactants.length > 0) {
            ...
            arrow = new kemia.model.Arrow(
				new goog.math.Coordinate(kemia.model.Reaction.MOLECULE_MARGIN, 0));
        }
        this.setArrow(arrow);
    }
    ...
    // products, as needed
    var mol_box_left = mol.getBoundingBox().left;
    var products = this.getProducts();
	var min_left;
	
	if(opt_permit_overlap){
		min_left = this.arrows[0].getCenter().x;
	} else if (products.length==0){
		min_left = this.arrows[0].target.x + kemia.model.Reaction.MOLECULE_MARGIN;
	} else {
		var prod_box = kemia.model.Reaction.boundingBox(products);
		min_left = prod_box.right + kemia.model.Reaction.MOLECULE_MARGIN;
	}

    if (min_left > mol_box_left) {
        mol.translate(new goog.math.Vec2(min_left - mol_box_left, 0));
        this.pluses.length = 0;
        // force plus generation, since we've moved the components
    }
...
/**
 * @return {Array.<kemia.model.Plus>} 
 */
kemia.model.Reaction.prototype.getPluses = function() {
    return this.pluses;
}
...
kemia.model.Reaction.boundingBox = function(molecules) {
    var atoms = goog.array.flatten(goog.array.map(molecules,
    function(mol) {
        return mol.atoms;
    }));
    var coords = goog.array.map(atoms,
    function(a) {
        return a.coord;
    })
    if (coords.length > 0) {
...
kemia.model.Reaction.prototype.translate = function(vector) {
    goog.array.forEach(this.molecules,
    function(mol) {
...
/** @inheritDoc */
kemia.model.Reaction.prototype.toString = function() {
    var msg = '\nReaction:';
    goog.array.forEach(this.getReactants(),
    function(m) {
        var c = m.getCenter();
        msg += '\n\tReactant: ' + c.x.toFixed(2) + ", " + c.y.toFixed(2);
    })

    goog.array.forEach(this.arrows,
    function(a) {
        var c = a.getCenter();
        msg += '\n\tArrow: ' + c.x.toFixed(2) + ", " + c.y.toFixed(2);
    })

    goog.array.forEach(this.getProducts(),
    function(m) {
        var c = m.getCenter();
        msg += '\n\tProduct: ' + c.x.toFixed(2) + ", " + c.y.toFixed(2);
    })

    goog.array.forEach(this.pluses,
    function(p) {
        var c = p.coord;
        msg += '\n\tPlus: ' + c.x.toFixed(2) + ", " + c.y.toFixed(2);
    })
