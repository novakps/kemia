...
/**
 * @param {kemia.model.Atom} atom to highlight
...
opt_element_array) {
    ...
    this.editorObject.reactionRenderer.moleculeRenderer.atomRenderer.highlightOn(
    atom, '#3366ff', opt_element_array);
    return opt_element_array;

};

/**
 * @param {kemia.model.Bond} bond to highlight
 * @param {kemia.graphics.ElementArray=} opt_element_array 
 * @return {kemia.graphics.ElementArray}
 */
...
opt_element_array) {
    ...
    .get(bond).highlightOn(bond, '#3366ff', opt_element_array);
    return opt_element_array;
};

/**
 * @param {kemia.model.Molecule} molecule to highlight
 * @param {kemia.graphics.ElementArray=} opt_element_array 
 * @return {kemia.graphics.ElementArray}
 */
...
opt_element_array) {
    ...
    this.editorObject.reactionRenderer.moleculeRenderer.highlightOn(molecule, color, opt_element_array);
    return opt_element_array;
};

/**
 * @param {kemia.model.Arrow} arrow to highlight
 * @param {kemia.graphics.ElementArray=} opt_element_array 
 * @return {kemia.graphics.ElementArray}
 */
...
opt_element_array) {
    ...
    return opt_element_array.add(this.editorObject.reactionRenderer.arrowRenderer.highlightOn(arrow,
    '#3366ff'));
};

/**
 * @param {kemia.model.Plus} plus to highlight
 * @param {kemia.graphics.ElementArray=} opt_element_array 
 * @return {kemia.graphics.ElementArray}
 */
...
opt_element_array) {
    ...
    return opt_element_array.add(this.editorObject.reactionRenderer.plusRenderer.highlightOn(plus,
    '#3366ff'));
};
...
target_atom) {
    ...
    goog.array.forEach(target_atom.bonds.getValues(),
    function(bond) {
    ...
    if (source_molecule !== target_molecule) {
        goog.array.forEach(source_molecule.atoms,
        function(atom) {
        ...
        // molecule
        goog.array.forEach(source_molecule.bonds,
        function(bond) {
            var new_bond = bond.clone();
...
kemia.controller.plugins.Move.closeRing = function(source_atom, target_atom) {
    ...
    var result = target_molecule.clone();
    goog.array.forEach(result.bonds,
    function(b) {
...
kemia.controller.plugins.Move.prototype.dragAtom = function(e, atom) {
    ...
    function(e) {
        if (d._hightlightElements) {
            goog.array.forEach(d._hightlightElements,
            function(g) {
                g.clear();
            });
        }
        d._hightlightElements = [];
        if (d.atom.molecule._elements) {
            d.atom.molecule._elements.clear();
        }
    ...
    function(e) {
        try {
            goog.array.forEach(d._merge_pairs,
            function(pair) {
...
kemia.controller.plugins.Move.prototype.dragMolecule = function(e, molecule) {
    ...
    function(e) {
        ...
        // move graphic
        d.molecule._elements.setTransformation(deltaX, deltaY, 0,
        0, 0);
...
kemia.controller.plugins.Move.prototype.dragCanvas = function(e) {
    ...
    function(e) {
        ...
        if (x || y) {
            ...
            / trans.getScaleX(), y
            / trans.getScaleY());
            goog.array.forEach(this.editorObject.getModels(),
            function(model) {
